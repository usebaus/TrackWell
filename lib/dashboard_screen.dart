...
class DashboardScreen extends StatelessWidget {
  ...
  Future<void> _showEditGoalsDialog(BuildContext context, GoalsProvider goals) async {
    final waterController = TextEditingController(text: goals.waterGoalMl.toString());
    final exerciseController = TextEditingController(text: goals.exerciseGoalMin.toString());
          ...
          children: [
            TextField(
              ...
              decoration: const InputDecoration(
                  labelText: 'Water base goal (ml)', hintText: 'e.g. 2000'),
            ),
            ...
            TextField(
              ...
              keyboardType: TextInputType.number,
              decoration: const InputDecoration(
                  labelText: 'Exercise goal (min)', hintText: 'e.g. 30'),
            ),
          ],
        ),
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              final water =
                  int.tryParse(waterController.text.trim()) ?? goals.waterGoalMl;
              final exercise =
                  int.tryParse(exerciseController.text.trim()) ?? goals.exerciseGoalMin;
  ...
  Widget build(BuildContext context) {
    ...
    // Dynamic water goal: base + (todaySteps / 1000 * 100), capped at base * 2
    final baseGoal = goals.waterGoalMl <= 0 ? 2000 : goals.waterGoalMl;
    final todaySteps = sp.todaySteps;
    final stepBonus = (todaySteps / 1000 * 100).round();
    final dynamicWaterGoal = (baseGoal + stepBonus).clamp(baseGoal, baseGoal * 2);
        ...
        child: Column(
          ...
          children: [
            ...
            if (goals.loading)
              ...
              Column(
                children: [
                  // Pass dynamic goal — includes step bonus
                  _WaterGoalCard(
                    uid: _uid,
                    dailyGoalMl: dynamicWaterGoal,
                    baseGoalMl: baseGoal,
                    todaySteps: todaySteps,
                    stepBonus: stepBonus,
                  ),
                  const SizedBox(height: 12),
                  _ExerciseGoalCard(uid: _uid, dailyGoalMin: goals.exerciseGoalMin),
                ],
              ),
...
class _ProfileSummaryCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final weight =
        profile.weightKg != null ? '${profile.weightKg!.toStringAsFixed(1)} kg' : '—';
                ...
                children: [
                  Text('BMI',
                      style: TextStyle(
                          ...
                          fontSize: 15,
                          color: AppTheme.textPrimary)),
                  Text('Add height & weight in Settings to calculate your BMI',
...
// ── Water Goal Card (now step-aware) ─────────────────────────────────────────
...
class _WaterGoalCard extends StatelessWidget {
  const _WaterGoalCard({
    ...
    required this.stepBonus,
  });

  final String uid;
  final int dailyGoalMl;   // dynamic (base + bonus)
      ...
      builder: (context, snapshot) {
        ...
        return Container(
          ...
          decoration: BoxDecoration(
              color: Colors.white,
              borderRadius: BorderRadius.circular(16),
              border: Border.all(color: reached ? Colors.green : AppTheme.border)),
          child: Column(
            children: [
              Row(
                children: [
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          '$todaySteps steps → +${stepBonus}ml bonus  •  Base ${baseGoalMl}ml + ${stepBonus}ml = ${goal}ml',
                          style: const TextStyle(
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasError) return _errorCard('Error loading exercise data');
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 6),
              Text(
                reached ? 'Goal reached! 🎉' : 'You can still move a bit',
                style: TextStyle(
                    fontSize: 13,
                    fontWeight: FontWeight.w500,
                    color: reached ? Colors.green : AppTheme.textMuted),
...
class _LiveStatChipsState extends State<_LiveStatChips> {
  ...
  void _listenAll() {
    ...
    _subs.add(db
        ...
        .listen((s) {
      if (mounted) {
        setState(() => _exerciseMins = (s.data()?['total_minutes'] as int?) ?? 0);
      }
    }));
  ...
  Widget build(BuildContext context) {
    final waterLabel =
        _waterMl >= 1000 ? '${(_waterMl / 1000).toStringAsFixed(1)}L' : '${_waterMl}ml';
